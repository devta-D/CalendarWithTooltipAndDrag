...
class DayView extends CheckedTextView {
    ...
    private int selectionColor = Color.parseColor(CalendarConstants.SELECTION_COLOR);
    ...
    public void setEndPoint(boolean endPoint) {
        if(endPoint){
            this.customBackground = generateEndDrawable();
        }else {
    ...
    private Drawable generateBackground(int color, int fadeTime, Rect bounds) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            drawable.addState(new int[]{android.R.attr.state_pressed}, generateCircleDrawable(Color.TRANSPARENT));
        }
    ...
    private Drawable generateEndDrawable(){
        GradientDrawable drawable = new GradientDrawable();
        drawable.setShape(GradientDrawable.RECTANGLE);
        drawable.setColor(Color.WHITE);
        drawable.setStroke(CalendarConstants.END_POINTS_BORDER_WIDTH, Color.parseColor(CalendarConstants.END_POINTS_BORDER_COLOR));
        return drawable;
    }
...
//        API 22. Technically harmless to leave on for API 21 and 23, but not worth risking for 23+
        ...
        return generateCircleDrawable(Color.TRANSPARENT);
    }
